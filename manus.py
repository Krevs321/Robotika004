...
class Manipulator(RemoteAPI):
    ...
    def __init__(self, server, name = "manipulator"):
        ...
        for i in range(n):
            if self.initial_types[i, 4]:
                self.initial_param[i, self.initial_types[i, 3].astype(np.int32) - 1] = self.state()[0][i]['position']
    ...
    def solve(self, position, iterations=40, distance=1):
        ...
        for k in range(iterations):
            ...
            for i in range(n):
                if self.initial_types[i, 4]:
                    parameters[i, self.initial_types[i, 3].astype(np.int32) - 1] = state[i]
    ...
    def ik_optimize_joint(self, position, start, joint, N):
        ...
        for i in range(nj):
            if self.initial_types[i, 4]:
                parameters[i, self.initial_types[i, 3].astype(np.int32) - 1, :] = start[i]

        parameters[joint, self.initial_types[joint, 3].astype(np.int32) - 1, :] = samples    
